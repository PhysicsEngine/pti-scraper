...
class DbUploader(object):
  ...
  def insert_articles_articles(self, content):
    ...
    with self.conn.cursor() as cur:
      ...
      cur.execute(sql, (content.pub_date, content.url, content.author_id, "dummy title"))
      articles_id = cur.lastrowid
      self.conn.commit()
  
    return articles_id

  def select_articles_authoers(self, name):
    with self.conn.cursor() as cur:
      sql = "SELECT id FROM articles_authers where name = '{0}'".format(name)
      cur.execute(sql)
      return cur.fetchone()
